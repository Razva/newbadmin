...
echo "Dumping SQL..."
mysqldump -u $sql_user -p$sql_pass $sql_db > $backup_location/$sql_db-$now.sql
echo "Compressing SQL..."
cd $backup_location && tar -czf $sql_db-$now-sql.tar.gz $sql_db-$now.sql
echo "Cleaning up SQL..."
rm -rf $backup_location/$sql_db-$now.sql
echo "Compressing directories..."
cd $backup_location && tar -czf files-$now.tar.gz $backup_directories
echo "DONE!"
